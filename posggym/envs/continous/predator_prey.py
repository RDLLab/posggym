...
from posggym.envs.grid_world.core import Coord, Position, Direction, ContinousWorld
...
class PPModel(M.POSGModel[PPState, PPObs, PPAction]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        def _coord_space():
            # return spaces.Tuple
            return spaces.Tuple(
                (spaces.Box(low=0, high=self.grid.width), spaces.Box(low=0, high=self.grid.height))
            )
        ...
        agent_obs = spaces.Tuple(_coord_space() for i in range(self.num_predators + self.num_prey))

        self.observation_spaces = {
            i: spaces.Tuple(
                tuple(
                    agent_obs
                    for _ in range(self.num_predators + self.num_prey)
                )
            )
    ...
    def _get_local_obs(
        ...
        obs += list(state.predator_coords[agent_id])
        for i in range(len(state.predator_coords)):
            if i == agent_id:
                continue
            obs += list(state.predator_coords[i])
        
        for i in range(len(state.prey_coords)):
            # print(state.prey_coords)
            obs += list(state.prey_coords[i])
...
class PPWorld(ContinousWorld):
    ...
    def __init__(
        self,
        grid_size: int,
        block_coords: Optional[Set[Position]],
        ...
        if predator_start_coords is None:
            ...
            predator_angles = [random.random() * math.pi for _ in range(len(predator_start_coords))]

        self.predator_start_coords = [x + (y,) for x,y in zip(predator_start_coords, predator_angles)]
