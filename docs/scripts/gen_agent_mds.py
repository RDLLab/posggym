...
for env_type, envs in tqdm(filtered_agents_by_env_type.items()):
    ...
    env_type_dir = os.path.join(agent_docs_dir, env_type.lower())

    if not os.path.exists(env_type_dir) and env_type != "generic":
        os.makedirs(env_type_dir)
    ...
    for env_name, pi_spec_list in envs.items():
        ...
        if env_type == "generic":
            env_md_path = os.path.join(agent_docs_dir, snake_env_name + ".md")
        else:
            env_md_path = os.path.join(env_type_dir, snake_env_name + ".md")
